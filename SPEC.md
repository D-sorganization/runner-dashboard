...
> [`docs/adr/`](docs/adr/). New significant decisions get a numbered ADR;
> the index in `docs/adr/README.md` is auto-generated by
> `scripts/regenerate-adr-index.py`.
...
### 2.5.14 - 2026-04-30
- docs: add the first five Architecture Decision Records under `docs/adr/`
  covering the Vite SPA migration, the HMAC dispatch envelope, the YAML
  lease store, the WebAuthn fail-closed scaffold, and the agent priority
  order. Index in `docs/adr/README.md` is regenerated by
  `scripts/regenerate-adr-index.py` (issue #398).
