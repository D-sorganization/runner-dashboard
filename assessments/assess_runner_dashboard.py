...
def main():
    ...
    if not pyproject.exists() and not req.exists():
        findings.append({"criterion":"A","severity":"P1","principle":"PP2","text":"No package manifest (pyproject.toml or requirements.txt)"})
    ...
    if total_funcs > 0 and docstring_funcs / total_funcs < 0.5:
        findings.append({"criterion":"B","severity":"P1","principle":"PP4","text":"Docstring coverage low: {}/{} public functions in backend/ sample".format(docstring_funcs, total_funcs)})
        score_b = max(score_b - 1, 0)
        ...
        try:
            ...
            for m in skip_pat.finditer(txt):
                ...
                if "issue" not in ctx.lower() and "#" not in ctx:
                    findings.append({"criterion":"C","severity":"P2","principle":"PP8","text":"pytest.skip/xfail without issue link in {}".format(f.name)})
        except:
            pass
    ...
    if bare_except:
        findings.append({"criterion":"D","severity":"P1","principle":"PP6","text":"Bare except blocks found in backend/"})
    ...
    for f in backend_py[:30]:
        try:
            ...
            pass
    if magic > 20:
        findings.append({"criterion":"F","severity":"P2","principle":"PP1","text":"Many magic float literals ({}) in backend/ sample".format(magic)})
    ...
    if "CVE" in audit:
        findings.append({"criterion":"G","severity":"P0","principle":"PP3","text":"pip-audit found CVEs in dependencies"})
    ...
    if "Issue:" in bandit:
        findings.append({"criterion":"H","severity":"P0","principle":"PP6","text":"bandit found HIGH/ MEDIUM severity issues"})
    ...
    if prints:
        findings.append({"criterion":"J","severity":"P2","principle":"PP6","text":"print() statements found in backend/ (use logging)"})
