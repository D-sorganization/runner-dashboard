...
class TestStripSensitive:
    def test_removes_known_sensitive_keys(self) -> None:
        raw = {
            ...
            "connection_string": "postgres://user:pass@host/db",  # pragma: allowlist secret
            "model": "claude-3-opus",
        }
    ...
    def test_recursive_strip(self) -> None:
        raw = {
            "backends": [
                {"name": "Ollama", "api_key": "hidden", "enabled": True},  # pragma: allowlist secret
            ]
        }
        result = mc.strip_sensitive(raw)
    ...
    def test_nested_dict_strip(self) -> None:
        raw = {
            "config": {
                "token": "t-secret",  # pragma: allowlist secret
                "host": "localhost",
            }
        }
        result = mc.strip_sensitive(raw)
    ...
    def test_unknown_keys_passthrough_before_model(self) -> None:
        ...
        raw = {"some_new_field": 42, "secret_token": "bad"}  # pragma: allowlist secret
        result = mc.strip_sensitive(raw)
...
class TestExtraFieldsDropped:
    ...
    def test_status_extra_fields_dropped(self) -> None:
        raw = {
            ...
            "active_tasks": 3,
            "queued_tasks": 1,
            "maxwell_internal_secret": "should-not-appear",  # pragma: allowlist secret
            "new_metric": "yes",
        }
