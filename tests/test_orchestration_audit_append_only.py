...
def _patched_audit_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    import orchestration_audit  # noqa: PLC0415

    audit_path = tmp_path / "orchestration_audit.json"
    monkeypatch.setattr(orchestration_audit, "ORCHESTRATION_AUDIT_PATH", audit_path)
    monkeypatch.setattr(orchestration_audit, "_audit_log_corrupt_total", 0)
    return audit_path
...
def test_5000_entries_readable_no_truncation(
    ...
    async def write_5000() -> None:
        for i in range(5000):
            await orchestration_audit.append_orchestration_audit({"event_id": f"e{i}", "i": i})

    asyncio.run(write_5000())

    all_entries = orchestration_audit.load_orchestration_audit(limit=10_000)
...
def test_limit_returns_latest_n(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    last_100 = orchestration_audit.load_orchestration_audit(limit=100)
...
def test_legacy_json_array_migrated_on_load(
    ...
    loaded = orchestration_audit.load_orchestration_audit(limit=100)
...
def test_corrupt_line_increments_counter(
    ...
    before = orchestration_audit.get_audit_log_corrupt_total()
    loaded = orchestration_audit.load_orchestration_audit(limit=10)
    after = orchestration_audit.get_audit_log_corrupt_total()
...
def test_append_does_not_rewrite_existing_lines(
    ...
    async def write_one(payload: dict) -> None:
        await orchestration_audit.append_orchestration_audit(payload)
...
def test_principal_filter(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def seed() -> None:
        for i in range(20):
            ...
            await orchestration_audit.append_orchestration_audit({"event_id": f"e{i}", "principal": who})

    asyncio.run(seed())

    alice_entries = orchestration_audit.load_orchestration_audit(limit=50, principal="alice")
    bob_entries = orchestration_audit.load_orchestration_audit(limit=50, principal="bob")
